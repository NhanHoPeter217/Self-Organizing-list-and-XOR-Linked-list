...
bool insertNewItem(self_list &L, int64_t new_value, int64_t pos)
{
...
bool buildLinkedList(self_list &L, vector <int64_t> list)
{
    for (int i = 0; i < list.size(); ++i) {
        if (insertNewItem(L, list[i], i) == false) return false;
    }
    return true;
}
...
void update_new_position(self_list &L, self_node* &pre_previous, self_node* &previous, self_node* &node)
{
    previous->next = node->next;
    node->next = previous;
    pre_previous->next = node;
}

bool search_self_list(self_list &L, int64_t val)
{
    if (L.head == NULL) return false;
    if (L.head->value == val) return true;
    if (L.head->next == NULL) return false;
    if (L.head->next->value == val) {
        self_node* tmp = L.head;
        self_node* node = L.head->next;
        tmp->next = node->next;
        node->next = tmp;
        L.head = node;
        return true;
    ...
    self_node* previous = L.head->next;
    self_node* pre_previous = L.head;

    for (self_node* p = L.head->next->next; p; p = p->next) {
        if (p->value == val) {
            update_new_position(L, pre_previous, previous, p);
            return true;
        }
...
int main()
{
    ...
    while (n != 8) {
        ...
        cout << "3. Insert a new item into the list\n";
        ...
        if (n == 1) {
            ...
            cout << "Position of item: "; cin >> pos;
            if (insertNewItem(L, value, pos)) cout << "Insert " << value << " successful!\n";
